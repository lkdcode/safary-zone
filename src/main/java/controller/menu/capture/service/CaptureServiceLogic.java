...
import static user.item.ball.MonsterBall.*;
...
public class CaptureServiceLogic {
    ...
    private boolean resultCalculator() {
        if (this.captureRate > getRandom(MIN_RANGE, MAX_RANGE)) {
            addWildPokemon();
            return true;
        }
    ...
    public boolean isNearSuccess() {
        if (getRandom(MIN_RANGE, MAX_RANGE) <= DEFAULT_GO_NEAR_SUCCESS_RATE) {
            successPoint += POINT;
            return true;
        }
        return false;
    ...
    public boolean isRun() {
        if (this.runRate <= getRandom(MIN_RANGE, MAX_RANGE)) return false;
        return true;
    }
