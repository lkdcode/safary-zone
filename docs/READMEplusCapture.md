...
> ## CaptureController
> 1. 랜덤으로 포켓몬스터를 조우한다.
>    - CaptureServiceLogic.getRandomWildEncounter();
>      - 야생 포켓몬을 랜덤으로 생성해서 호출
> 2. 총 3가지의 매뉴를 보여주고 선택을 입력받는다.
> - OutPutView(콘솔 출력), InputPutView(입력 담당)
> - OutPutView.menu() : 3가지 메뉴를 콘솔에 출력 (1.포획, 2.다가가기, 3.도망감)
> - InputView.menu() : 유저한테서 어떤 메뉴를 실행할지 입력을 받는다
> - (CaptureServiceLogic에서 총괄 관리)
> - 볼 던지기를 선택한 경우 ( CaptureServiceLogic.throwBall(); )
>     - 세 종류의 볼을 콘솔에 출력한다. (OutputView.selectBall();)
>     - 어떤 볼을 던질지 (몬스터볼, 하이퍼볼, 마스터볼) 입력받는다. (InputView.selectBall();)
>     - 해당 볼의 수량을 확인한다. (InputValidate)
>     - 해당 볼을 던질지 확인한다. (ex.몬스터볼을 던지시겠습니까?)(InputValidate)
>     - 던지기를 선택했으면 해당 볼의 포획 확률을 계산한다. (CaptureServiceLogic)
>     - 포획에 성공하면 잡았다는 메세지를 출력한다. (OutputView.captureResult();)
>     - CaptureMain으로 돌아간다. (return 처리)
> - 다가가기를 선택한 경우 ( CaptureServiceLogic.goNear(); )
>   - 플레이어가 다가간다는 메세지를 출력 (OutputView.goNearPokemon();)
>   - 다가가면 이후 볼의 포획확률을 늘려준다. (CaptureServiceLogic)
>   - 다가가면 랜덤한 확률로 포켓몬이 바로 도망치는 상황이 발생한다. (CaptureServiceLogic)
> - 도망가기를 선택한 경우 ( 바로 종료해서 return )
>   - 포획 상황을 종료한다.
